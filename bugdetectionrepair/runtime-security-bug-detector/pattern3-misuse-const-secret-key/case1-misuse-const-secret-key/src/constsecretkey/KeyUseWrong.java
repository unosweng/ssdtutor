...
public class KeyUseWrong {
   ...
   public static void main(String[] args) {
      try {
         ...
         // Sometimes a SHA function can be used to generate these 16 bytes
         // from any string in arbitrary length
   ...
   void foo() {
      try {
         SecretKeySpec keyspec = new SecretKeySpec(secretInField.getBytes(), "AES");
         byte[] iv = new byte[128 / 8];
         SecureRandom prng = new SecureRandom();
         prng.nextBytes(iv);
   ...
   void bar() {
      try {
         SecretKeySpec keyspec = new SecretKeySpec("THISISASECRET!!!".getBytes(), "AES");
         byte[] iv = new byte[128 / 8];
         SecureRandom prng = new SecureRandom();
         prng.nextBytes(iv);
         Cipher cipher = Cipher.getInstance(algorithm); // Select a cipher
