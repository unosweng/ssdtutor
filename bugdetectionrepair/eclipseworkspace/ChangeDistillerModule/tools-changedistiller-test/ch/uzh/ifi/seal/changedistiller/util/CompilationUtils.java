...
public final class CompilationUtils {
	...
	private static final String TEST_DATA_BASE_DIR = "resources" + File.separator + "testdata" + File.separator;
	...
	private static Parser createCommentRecorderParser(CompilerOptions options) {
		Parser parser = new CommentRecorderParser(new ProblemReporter(DefaultErrorHandlingPolicies.proceedWithAllProblems(), options, new DefaultProblemFactory()), false);
		return parser;
	}
	...
	/**
	 * Returns the generated {@link JavaCompilation} from the file identified by the given filename. This method assumes that the filename is relative to <code>{@value #TEST_DATA_BASE_DIR}</code>.
	 * 
	 * @param filename
