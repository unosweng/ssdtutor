   ...
   void defUseAnalysisBackwardSlice(CFGNode n) throws Exception {
      ...
      if (n.isRoot() && -1 == n.getParmPositionToBackTrack()) {
         ...
         if (n.getMethodDec() != null && object != null) {
            ...
            detector.checkVdfSetContains(varObj, true);
         }
      ...
      Map<String, VariableDeclarationFragment> variableDecl = new HashMap<String, VariableDeclarationFragment>();
      CompilationUnit iCUnit = UtilAST.parse(n.iMethodCallee.getCompilationUnit());
      iCUnit.accept(new ASTVisitor() {
         public boolean visit(VariableDeclarationFragment vdf) {
            variableDecl.put(vdf.resolveBinding().toString(), vdf);
            return true;
         }
      });
      iCUnit = null;
      System.gc();
      n.variableDeclFragment = variableDecl;
      // # Trace backward from a call site with an affecting parameter.
      n.findBackwardSlices(detector, ideMetInv, variableDecl);
   }
   ...
   protected int findMatchedParameter(List<?> parameters, SimpleName spName) {
      ...
      for (int i = 0; i < parameters.size(); i++) {
         ...
         if (iObj instanceof SingleVariableDeclaration) {
            ...
            if (spName.getFullyQualifiedName().equals(varName.getFullyQualifiedName())) {
               index = i;
               break;
            }
         }
