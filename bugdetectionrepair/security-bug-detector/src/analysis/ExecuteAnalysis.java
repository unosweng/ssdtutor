...
public class ExecuteAnalysis {
   ...
   public ExecuteAnalysis(DetectionASTVisitor detector, int pattern) {
   ...
   public int go(EPartService epartService, boolean clear) {
      ...
      if (epartService.findPart(CryptoMisuseDetectionTree.VIEW_ID) != null) {
         ...
         if (findPartObjTree instanceof CryptoMisuseDetectionTree) {
             ...
             ProgramTreeAnalyzer analyzer = new ProgramTreeAnalyzer(v, this.detector);
             analyzer.analyze();
     ...
     if (epartService.findPart(CryptoMisuseDetection.VIEW_ID) != null) {
         ...
         if (findPartObj instanceof CryptoMisuseDetection) {
             ...
             ProgramAnalyzer analyzer = new ProgramAnalyzer(v, this.detector);
             analyzer.analyze();

             // tree list
             count = v.getTreeList().size();
             System.out.println(v.getTreeList().size());
             if (count > 0) {
                 PrintHelper.printDebugMsgH("Tree Lists: " + count);
